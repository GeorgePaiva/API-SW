...
class ApiSwApplicationTests {
	...
	public void testa_Busca_Nome() {
		...
		ResponseEntity<String> response = rest.postForEntity(BASE_PATH + port + "/planetas/", planeta, String.class);
		ResponseEntity<String> respostaBusca = rest.getForEntity(BASE_PATH + port + "/planetas/buscanome?nome=Anakin",
				String.class);
		Assert.assertEquals(200, respostaBusca.getStatusCodeValue());
	...
	public void testa_Busca_Nome_Se_Nao_Existe() {
		ResponseEntity<String> response = rest.getForEntity(BASE_PATH + port + "/planetas/buscanome?nome=Teste",
				String.class);
	...
	public void testa_Deletar() {
		...
		ResponseEntity<String> respostaBusca = rest.exchange(response.getHeaders().getLocation().toString(),
				HttpMethod.DELETE, criaHeader(), String.class, planeta);
